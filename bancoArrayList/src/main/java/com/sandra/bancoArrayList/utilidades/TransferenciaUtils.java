...
public class TransferenciaUtils {
	...
	public static void getTransferencia(List<Transferencia> transferencias) {
	...
	// beneficiario)
	public static int comprobarId(List<Cliente> clientes, String tipo) {
		sc = new Scanner (System.in);
	...
	public static void addTransferencia(List<Transferencia> transferencias, List<Cliente> clientes, int id_ordenante,
		...
		if(id_ordenante == id_beneficiario) {
			...
			if (superaImporteMaximo(clientes, id_ordenante, importe)) {
				System.err.println("No se pudo completar la transferencia. Se le redigirá al Menú Transferencias.");
				transferencias.remove(id);
			} else {
	...
	private static boolean superaImporteMaximo(List<Cliente> clientes, int id_ordenante, double importe) {
		sc = new Scanner (System.in);
	...
	private static void cambiarSaldosClientes (List<Cliente> clientes, int id_ordenante, int id_beneficiario, double importe) {
		...
		JOptionPane.showMessageDialog(null, "Nuevo saldo del cliente ordenante (ID: "+(++id_ordenante)+"): "+saldoO+"€"
				+ "\nNuevo saldo del cliente beneficiario (ID: "+(++id_beneficiario)+"): "+saldoB+"€", "TRANSFERENCIA REALIZADA CORRECTAMENTE", 1, new ImageIcon("src/main/java/com/sandra/bancoArrayList/images/dinero.png"));
